...
//    }
    ...
    //最初にローカルプロパティを全て初期化する。
	void compLocalProperty(LirNode exp, LirNode addr, ArrayList vars,int blkid,int inst){
		xSameAddr = new boolean[idBound];
		nSameAddr = new boolean[idBound];
		nUse = new boolean[idBound];
		xUse = new boolean[idBound];
...
//		System.out.println("exp:"+exp);
		for(int i=1;i<bVecInOrderOfRPost.length; i++) {
			...
			nUse[blk.id] = compNUse(exp,addr,vars,blk);//〇
			xUse[blk.id] = compXUse(exp,addr,vars,blk);//〇
			
			nIsSame[blk.id] = compNIsSame(exp,vars,blk);//〇
			xIsSame[blk.id] = compXIsSame(exp,vars,blk);//〇
			
			nSameAddr[blk.id] = isNSameAddr(exp,vars,blk);
			xSameAddr[blk.id] = isXSameAddr(exp,vars,blk);
			
			//変数のkillが内科のチェック
			Transp_e[blk.id] = compTranspe(exp,addr,vars,blk,blkid,inst);
			//配列のアクセス順序が崩れていないかのチェック。
			Transp_addr[blk.id] = compTranspAddr(exp,addr,vars,blk,blkid,inst);
			//
...
//			System.out.println("");
//			System.out.println("blk.id:"+blk.id);
//			if(nUse[blk.id]) System.out.println("nUse");
//			if(xUse[blk.id]) System.out.println("xUse");
//			if(nIsSame[blk.id]) System.out.println("nIsSame");
//			if(xIsSame[blk.id]) System.out.println("xIsSame");
//			if(nSameAddr[blk.id]) System.out.println("nSameAddr");
//			if(xSameAddr[blk.id]) System.out.println("xSameAddr");
...
//		}
	 ...
	 * @return
	 */
	private boolean isNSameAddr(LirNode exp, ArrayList vars, BasicBlk blk) {
		for (BiLink p = blk.instrList().first(); !p.atEnd(); p = p.next()) {
			LirNode node = (LirNode) p.elem();
	 ...
	 * @return
	 */
	private boolean isXSameAddr(LirNode exp, ArrayList vars, BasicBlk blk) {
		for (BiLink p = blk.instrList().last(); !p.atEnd(); p = p.prev()) {
			LirNode node = (LirNode) p.elem();
			if (isKill(exp,node,vars,blk,p)) {
				return false;
...
//			if(isStore(node)) System.out.println("kid0 same?"+node.kid(0).equals(exp));
			...
			if(isLoad(node) && node.kid(1).equals(exp)) {
				return true;
			}
			
			if(isStore(node) && node.kid(0).equals(exp)) {
				return true;
			}
		}
//		System.out.println("--FalseFalseFalseFalse--");
...
//		System.out.println("--FalseFalseFalseFalse--");
	...
	private boolean compNUse(LirNode exp, LirNode addr, ArrayList vars, BasicBlk blk){
//		System.out.println("::XisSame"+blk.id);//
		for(BiLink p=blk.instrList().first();!p.atEnd();p=p.next()){
			LirNode node = (LirNode)p.elem();
//			System.out.println("");
...
//		System.out.println("--FalseFalseFalseFalse--");
	...
	private boolean compXUse(LirNode exp, LirNode addr, ArrayList vars, BasicBlk blk){
//		System.out.println("@compXUse. blk:"+blk.id);
...
//			System.out.println("");
//			System.out.println("** " + blk.id + "**");
//			System.out.println("nDSafe:"+nDSafe[blk.id]);
//			System.out.println("xDSafe:"+xDSafe[blk.id]);
//		}
//		for(BiLink p=f.flowGraph().basicBlkList.first();!p.atEnd();p=p.next()){
//			BasicBlk blk = (BasicBlk)p.elem();
//			System.out.println("++++++++++++++++++++++++++");
...
//		System.out.println("++++"+xt+"++++");
	...
	//同様のストア命令に対する変更も、同様の配列と同様の番地へのロード命令もなければtrue;
	private boolean compTranspAddr(LirNode exp, LirNode addr, ArrayList vars, BasicBlk blk,int blkid,int inst){
//		System.out.println("::compTranspAddr");//
		if(!Transp_e[blk.id])return false;
		int number = 0;
		for(BiLink p=blk.instrList().first();!p.atEnd();p=p.next()){
...
//				System.out.println("^^^^false^^^^");//
	...
	private boolean compXTranspAddr(LirNode exp, LirNode addr, ArrayList vars, BasicBlk blk,int blkid,int inst){
		int number = inst;
//		System.out.println("");
//		System.out.println("@compXTranspAddr");
		for(BiLink p=blk.instrList().last();!p.atEnd();p=p.prev()){
			LirNode node = (LirNode)p.elem();
			number-=1;
...
//			System.out.println("exp:"+exp);
//			System.out.println("node:"+node);
//			System.out.println("vars.contains@isKill?"+vars.contains(node.kid(0)));
//			System.out.println("isStore(node) && !sameAddress(node,expr)?"+(isStore(node) && !sameAddress(node.kid(0),exp)));
//			System.out.println("isKill?"+isKill(exp,node,vars,blk,p));
//			if(blk.id==blkid&&number==0)continue;
...
//				boolean n = xEarliest[blk.id] || x;
	...
	boolean localCM(LirNode expr, LirNode addr, ArrayList vars, BasicBlk blk, BiLink p){
		...
		for(BiLink q=p.next();!q.atEnd();q=q.next()){
			LirNode node = (LirNode)q.elem();
			if(!isStore(node)&&isKill(expr.kid(0),node,vars,blk,p)) {
				System.out.println("isKill");
				return false;
			}
			ArrayList nvars = new ArrayList();
...
//				LirNode addr = getAddr(node.kid(1));
				LirNode addr = getAddr(node.kid(0));//〇getadd
//				System.out.println("addr:"+addr);
//				System.out.println("FRAME?:"+(addr.opCode != Op.FRAME));
				if(addr.opCode != Op.FRAME) continue;
...
//				if(newNode!=null) replace(newNode);
	...
	public void pde(LirNode node, LirNode addr, ArrayList vars, BasicBlk blk,BiLink p,int inst) {
...
//			compReplace();
	...
	public boolean dce(LirNode exp, LirNode addr, ArrayList vars, BasicBlk blk,int inst) {
        //for文でIsSameを各ノードに適用させながら、compDSafeを適用させ、除去できるかを判定。dceに結果を格納する。
        //exitノードで結果がtrueだったのなら除去可能。
		compLocalProperty(exp,addr,vars,blk.id,inst);
		compDSafe();
		compPartialDSafe();
		compEarliest(blk);
...
//		System.out.println("xDSafe[blk.id]:"+xDSafe[blk.id]);
//		System.out.println("xTransp_addr[blk.id]:"+xTransp_addr[blk.id]);
//		System.out.println("xUse[blk.id]:"+xUse[blk.id]);
//		System.out.println("isTarget?"+isTarget);
	...
	public boolean dce(BasicBlk blk) {
//		System.out.println("");
//		System.out.println("@check isTarget");
//		if(!xDSafe[blk.id] && xEarliest[blk.id] && xTransp_addr[blk.id] && !xUse[blk.id]) return true;
		if(xEarliest[blk.id] && xTransp_addr[blk.id] && !xUse[blk.id]) {
			for(BiLink p=blk.succList().first();!p.atEnd();p=p.next()) {
				BasicBlk succ = (BasicBlk)p.elem();
//				System.out.println("");
//				System.out.println("succ:"+succ.id);
//				System.out.println("pNDSafe[succ.id]:"+pNDSafe[succ.id]);
				if(pNDSafe[succ.id]) {
					return false;
				}
			}
